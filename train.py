...
def train():
    ...
    if not os.path.exists('./checkpoints/' + opt.expID):
        os.makedirs('./checkpoints/' + opt.expID)
    
    results = open('./checkpoints/' + opt.expID + "/validResults.txt", "a+")
    best_mIoU = 0
    best_idx = 0
    best_F1_or_dice = 0
    for epoch in range(opt.nEpoch):
        ...
        print(f'------ Epoch {epoch + 1} ------ Time: {current_time}')
        ...
        if ((epoch + 1 + 0) % opt.ckpt_period == 0): 
            torch.save(model.state_dict(), './checkpoints/' + opt.expID+"/ck_{}.pth".format(epoch + 1 + 0))
        
        
        #if metrics_result['IoU_mean'] > best_mIoU:
        if metrics_result['F1'] > best_F1_or_dice:
            best_idx = epoch + 1 + 0
            # best_mIoU = metrics_result['IoU_mean']
            best_F1_or_dice = metrics_result['F1']
            
            torch.save(model.state_dict(), './checkpoints/' + opt.expID+"/ck_{}.pth".format(epoch + 1 + 0))
        print("Epoch %d / %d with best F1_or_dice: %.4f" % (best_idx, opt.nEpoch, best_F1_or_dice))
    print("\nEpoch %d with best F1_or_dice: %.4f" % (best_idx, best_F1_or_dice), file=results)
